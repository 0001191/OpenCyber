      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            // Inject analysis plan if present
            const planPath = path.join(ctx.worktree, ".analysis-plan.json")
            try {
              const planExists = yield* Effect.promise(() => 
                import("fs/promises").then(fs => fs.access(planPath).then(() => true).catch(() => false))
              )
              if (planExists) {
                const planContent = yield* Effect.promise(() =>
                  import("fs/promises").then(fs => fs.readFile(planPath, "utf-8"))
                )
                system.push(`## Current Analysis Plan\n\n\`\`\`json\n${planContent}\n\`\`\``)
              }
            } catch { /* plan file is optional */ }
