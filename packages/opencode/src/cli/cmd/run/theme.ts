...
function splashTheme(theme: TuiThemeCurrent, indexed: RGBA[]): RunSplashTheme {
  const matrixGreen = RGBA.fromHex("#00ff41")
  const left = nearestIndexed(indexed, matrixGreen)
  const right = nearestIndexed(indexed, matrixGreen)
  return {
    left,
    right,
...
export const RUN_THEME_FALLBACK: RunTheme = {
  ...
  splash: {
    left: RGBA.fromHex("#00ff41"),
    right: RGBA.fromHex("#00ff41"),
    leftShadow: splashShadow(fallbackSplashIndexed, RGBA.fromValues(0, 0, 0, 0), RGBA.fromHex("#00ff41"), 0.14),
    rightShadow: splashShadow(fallbackSplashIndexed, RGBA.fromValues(0, 0, 0, 0), RGBA.fromHex("#00ff41"), 0.14),
  },
  block: {
    text: seed.text,
    muted: seed.muted,
